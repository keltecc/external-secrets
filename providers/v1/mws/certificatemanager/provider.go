...
func (p *Provider) NewClient(
	...
	spec := store.GetSpec()
	projectID := spec.Provider.MWSCertificateManager.ProjectID
...
func (p *Provider) ValidateStore(store esv1.GenericStore) (admission.Warnings, error) {
	...
	if spec == nil || spec.Provider == nil || spec.Provider.MWSCertificateManager == nil {
		return nil, errors.New("certificate manager spec is not provided")
	}

	certificateManagerProvider := spec.Provider.MWSCertificateManager

	if certificateManagerProvider.Auth.AuthorizedKey.Name == "" {
		return nil, errors.New("invalid spec: auth.authorizedKey is required")
	}

	if certificateManagerProvider.ProjectID == "" {
		return nil, errors.New("invalid spec: projectID is required")
	}

	err := esutils.ValidateReferentSecretSelector(store, certificateManagerProvider.Auth.AuthorizedKey)
	if err != nil {
